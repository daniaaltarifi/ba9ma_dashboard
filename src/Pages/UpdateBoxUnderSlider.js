...
function UpdateBoxUnderSlider() {
  const navigate = useNavigate();
  const location = useLocation();
  const [boxsliderId, setBoxSliderId] = useState("");
  const [title, setTitle] = useState("");
  const [descr, setDescr] = useState("");
  
  const [boxslider, setBoxSlider] = useState([]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setBoxSlider((prevAdd) =>
        prevAdd.map((data) => (data.id === boxsliderId ? response.data : data))
      );
      ...
      <div className="container">
        <div className="row">
          <div className="col-lg-2 col-md-6 col-sm-12">
            <div className="title_add_course">تعديل الرئيسية</div>
          </div>
        </div>
...
export default UpdateBoxUnderSlider;
