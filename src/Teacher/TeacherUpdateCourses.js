...
const [currentContext, setCurrentContext] = useState(null); // or 'link'
 ...
 const fetchDepartments = async () => {
  ...
  const fetchLinks = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/courses/links/${updateTeacherId}`);
  ...
  const fetchVideos = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/courses/videos/${updateTeacherId}`);
  ...
  const getTeacherCourseById = async (teacherEmail, updateTeacherId) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/teacher/teacher-course/${updateTeacherId}?teacherEmail=${encodeURIComponent(teacherEmail)}`
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8080/teacher/updatecourseteacher/${updateTeacherId}`,
        formData,
        {
          headers: {
